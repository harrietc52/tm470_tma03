# Section 1: Draft Project report (3737/3000)
...
Table x: Scope and evaluation criteria of prioritisation (Moscow Appendix 3)
...
To allow client to server communication, the server binds to 0.0.0.0, which allows all ingress traffic, including the Expo emulator. This is a security hole, and if developed to scale, firewalls, ACLs and ports would need to be configured and Cross Origin Requests (CORS) restricted (T. Open University n.d.). An API key between the server and client could also be implemented, allowing the server to authenticate incoming requests. 

In regards to social impact, the system will positively contribute to sustainability, by preventing waste and unnecessary transportation of food, having a positive impact on climate issues, by reducing the carbon footprint of food consumed. [@jones_why_2021]. Additionally, allotment growers have claimed to save around £950 a year through home growing. (R. Jones 2009) (Note: save costs could have increased since 2009, due to food prices increase.) However, this increase in home growing could change growers buying patterns, leading to challenges for small local businesses that grow and sell vegetables. These businesses may need to increase prices, which could negatively impact people who don’t grow vegetables. To avoid this, schemes to support and promote local business could be considered.

If the result is more people grow vegetables, this also has mental health benefits (H. Jones 2021). Especially during Covid-19, where home growers had higher well-being and lower levels of food insecurity (Mead et al. 2021). As well as boosting mental wellbeing, growing vegetables also provides physical exersise, as soil is maintained, and crops are cared for, resulting in alot of time spend outdoors in fresh air. Most of all, the imact would hopefully be the reduced time it takes growers to plan what they would like to plant that year. Specifically, the timeline feature might help growers be more aware of what jobs need to be done when, reducing the congnitive load of groweing vegetables and incresing the enjoyment. 
...
After three weeks the questionnaire was closed and the "Main Takeaways" summarised in Appendix 13. These were stand-out requirements that made up the applications Use Cases (Figure x). Use Cases, in the form of user stories with textual descriptions, functional and non-functional requirements, were created (ref textual description OU).  Figure x shows one example of a user story and Appendix 14 includes others. These use cases served as the basis for the functional requirements in the Volere template (ref volere template OU) in Appendix 15. 
...
Figure x: Mock ups (Timetable mock up in Appendix 18)

#### Implementation

The figures below show various forms of results from the current working system, with more evidence in Appendix 20 in the form of `git commit` messages, and project structure (Appendix 21) and code (Appendix 22). Further implementation details and learnings are shared in Section x.x


![before ux.png](../../Zettlr_Images/before ux.png "before ux.png")

Figure x: After implementing the MVP stories, the functionality was created but the interface was not very attactive. Figure x shows the application using only HTML. 

![basic css.png](../../Zettlr_Images/basic css.png)

Figure x: Some basic CSS was implemented (Figure x) but it was quickly apparently an Styling library was required. 
...
Figure x shows screenshots of the current working system.  The process of choosing and implementing a UX library is described in section x.x. The results of the system, implementing a styling library [@noauthor_icon_nodate] is shown in Figure x.x.


![db schema.png](../../Zettlr_Images/db schema.png)
Figure x: shows the current rails database schema
...
Where possible, stakeholders were asked to interact with the system through the emulator and others sent screenshots of the application (as show in Figure x). Stakeholder feedback is summarised in the below Figure x. Section x.x describes how this feedback was converted into stories for the next iteration. 
